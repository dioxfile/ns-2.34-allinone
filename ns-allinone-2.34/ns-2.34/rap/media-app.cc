...
//
// $Header: /cvsroot/nsnam/ns-2/rap/media-app.cc,v 1.14 2005/08/25 18:58:10 johnh Exp $
...
MediaSegmentList MediaSegmentList::check_holes(const MediaSegment& s)
{
	...
	while ((tmp != NULL) && (tmp->overlap(s))) {
		if (soff < tmp->start()) {
			...
			res.add(MediaSegment(soff, min_(eoff, tmp->start())));
#if 1
			// DEBUG ONLY
			// Check if these holes are really holes!
			if (in(MediaSegment(soff, min_(eoff, tmp->start())))) {
				fprintf(stderr, "Wrong hole: (%d %d) ", 
					soff, min_(eoff, tmp->start()));
...
int MediaSegmentList::overlap_size(const MediaSegment& s) const
{
	...
	eoff = s.end();
	while ((tmp != NULL) && (tmp->overlap(s))) {
		res += min_(eoff, tmp->end()) - max_(soff, tmp->start());
		soff = tmp->end();
		tmp = tmp->next();
	}
	return res;
...
rate: %.3f, avgrate: %.3f, srtt:%.3f\n", rate, avgrate_, srtt);
	...
	if((layers == 1) && (playing_[0] != 1)){
		...
		bufneeded = max_(4*BufNeed((LAYERBW_-rate/2.0), slope), 
				2*MWM(srtt));
